...
std::string toString(queue *Q){
  ...
  {
    //Return empty queue if first and last nodes are nullptr
    return "[]";
  }
  else if (Q->first == Q->last)
  {
    //Return queue if it only has one element
    return "[(" + std::to_string(Q->first->data) + ")]";
  }
  else
  {
    node *current = Q->first;
    std::string s = "[(" + std::to_string(current->data) + ") ";
    current = current->next;
    //Traverse up to the end
